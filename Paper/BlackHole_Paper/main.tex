...
\textcolor{red}{Before projecting to observables, we verify that the launched wind is physically well-behaved. Figure~\ref{fig:gaia_windstruct} presents three diagnostics measured in the frame of the mass-losing companion. The left panel shows the mass flux $\dot{M}(r) = \sum_i m_i v_{r,i}/\Delta r$ through radial shells: for a steady radial wind this should equal the launch rate at all radii, and indeed the measured flux hovers about the Wood-calibrated $\dot{M} \approx 1.2\times 10^9$~kg~s$^{-1}$ (dashed line) across the freely-expanding region ($2$--$15$~AU), falling off only beyond the wind front where material has not yet arrived in the integration window. The centre panel shows the speed relative to the companion rising toward the Parker asymptotic value $v_\infty \approx 830$~km~s$^{-1}$ (equation~\ref{eq:parker_vinf}), confirming the launch kinematics. The right panel gives the FIP-corrected per-species mass-loss budget (equation~\ref{eq:fip}): hydrogen dominates at $74\%$ by mass, with the low-FIP metals Fe and Si enhanced relative to their photospheric abundances.}

\doublefig
    {gaia_windstruct}
    {Figures/gaiabh1_wind_structure.png}
    {0pt}
    {0pt}
    {1.0}
    {\textcolor{red}{Wind-structure diagnostics for the Gaia~BH1 G-dwarf wind, measured in the companion frame. \emph{Left:} radial mass-flux $\dot{M}(r)$ through spherical shells; the dashed line is the launch rate. The near-constant flux across $2$--$15$~AU demonstrates mass conservation, with the drop-off beyond marking the wind front at $T_{\rm sim} = 66$~d. \emph{Centre:} particle speed relative to the companion versus distance, with the median (black) approaching the Parker asymptotic velocity $v_\infty \approx 830$~km~s$^{-1}$ (dashed). Colours denote species. \emph{Right:} FIP-corrected mass-loss rate per species; the dashed line marks the total $\dot{M} = 1.2\times 10^9$~kg~s$^{-1}$. Integrator: REBOUND \cite{Rein_Liu_2012} + REBOUNDx \texttt{gr\_full} \cite{Tamayo_2020}, MERCURIUS hybrid \cite{ReinTamayo2019}.}}
